class Chessboard {
			...
			if (Object.keys(piece).length === 0) {
				...
				if (piece.color === this.currentTurn) {
					...
					const fromIndex = [
						...
						this.selectedPiece.colIndex,
					];
					const toIndex = [rowIndex, colIndex];
					const valid = this.isMoveLegal(fromIndex, toIndex);
					if (valid) {
						//algebraic chess notation
						const fromCoord = this.getCoordsFromArrayIndex(
							fromIndex[0],
							fromIndex[1]
						);
						const toCoord = [this.selectedSquare.row, this.selectedSquare.col];
	...
	movePieceBoard(fromCoords, toCoords) {
		...
		const fromSquare = document.querySelector(
			`.square[data-row="${fromCoords[0]}"][data-col="${fromCoords[1]}"]`
		);
		const toSquare = document.querySelector(
			`.square[data-row="${toCoords[0]}"][data-col="${toCoords[1]}"]`
		);
		//move the piece
	...
	isCheckmate(player) {
		...
		if (!this.isKingSafe(player)) {
			...
			for (let i = 0; i < this.moves.length; i++) {
				...
				if (
					move.piece.color === player &&
					this.isMoveLegal(move.from, move.to)
				) {
					const move = this.moves[i];
					if (this.isKingSafe(player)) {
    ...
    filterObstructedMoves(piecePosition, possibleMoves){
        let filteredMoves = [];
        ...
        for (let square of possibleMoves) {
          ...
          if (row >= 0 && row < 8 && col >= 0 && col < 8) {
            let pieceAtSquare = this.grid[row][col];

            // Check for obstruction (occupied square)
            if (!pieceAtSquare) {
              // Empty square, add it to possible moves
              filteredMoves.push(square);
            } else {
              // There's a piece on the square, check for capture
              // Assume you have a function isOpponentPiece(piece1, piece2) to check if two pieces are of opposite colors
              if (this.currentTurn != this.grid[row][col]) {
                // Opponent's piece, add it to possible captures
                filteredMoves.push(square);
              }

              // Stop further checking in this direction (can't move through pieces)
              break;
            }
          }
        }
	...
	highlightPossibleMoves() {
		const moves = this.filterObstructedMoves(this.selectedPiece.position, this.selectedPiece.getPossibleMoves());
		moves.forEach((move) => {
            console.log(move)
			...
			const square = document.querySelector(
				`.square[data-row="${rowCoord}"][data-col="${colCoord}"]`
			);
...
// making a new branch
// that made a new branch (-b_ and went to it (checkout))
//so now you can change your code and commit it to the new branch